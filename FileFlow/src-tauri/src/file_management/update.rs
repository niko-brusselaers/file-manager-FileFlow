...
pub async fn rename_item(
    ...
    let new_file_path =
        gen_available_file_name(PathBuf::from(format!("{}/{}", file_path, new_name))).await;
...
pub async fn move_item(old_path: String, new_path: String) -> Result<u64, String> {
    ...
    match PathBuf::from(old_path.clone()).is_file() {
        true => {
            ...
            let result = fs_extra::file::move_file(&old_path, &new_path, &copyoptions)
        ...
        false => {
            ...
            let result = fs_extra::dir::move_dir(&old_path, &new_path, &copyoptions)
                .map_err(|e| e.to_string())?;
            Ok(result)
        }
    }
...
pub async fn copy_item(old_path: String, new_path: String) -> Result<u64, String> {
    ...
    match old_path.is_file() {
        true => {
            let copyoptions = fs_extra::file::CopyOptions::new().overwrite(true);
            let result = fs_extra::file::copy(&old_path, &new_path, &copyoptions)
                .map_err(|e| e.to_string())?;
            Ok(result)
        }
        false => {
            let copyoptions = fs_extra::dir::CopyOptions::new();
            std::fs::create_dir(new_path.clone()).map_err(|e| e.to_string())?;
            let result = fs_extra::dir::copy(&old_path, &new_path, &copyoptions)
                .map_err(|e| e.to_string())?;
            Ok(result)
        }
    }
