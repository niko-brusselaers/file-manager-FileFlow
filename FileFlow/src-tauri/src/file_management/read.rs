...
pub fn check_path(path: String) -> Result<File, String> {
    ...
    let hidden =
        get_hidden_property(PathBuf::from(path.clone())).map_err(|error| error.to_string())?;
...
pub fn search_device(query: &str, limit: usize) -> Result<Vec<File>, String> {
    ...
    let drive_paths = drives
        .iter()
        .map(|drive| drive.path.to_str().unwrap().to_string())
        .collect::<Vec<String>>();
    ...
    thread::spawn(move || -> Result<(), String> {
        ...
        tx.send(files)
            .map_err(|_| "Error sending data from thread".to_string())
    });

    let files = rx
        .recv()
        .map_err(|_| "Error receiving data from thread".to_string())?;
